		...
		<view class="login">
          ...
          <view class="login__mode__slider tn-cool-bg-color-15--reverse" :style="[modeSliderStyle]"></view>
				...
				<view class="login__info tn-flex tn-flex-direction-column tn-flex-col-center tn-flex-row-center">
					...
					<block v-if="currentModeIndex === 0">
						...
						<view
							...
							<view
								...
								<input v-model="password" placeholder-class="input-placeholder" placeholder="请输入密码" />
							</view>
					...
					<block v-if="currentModeIndex === 1">
						...
						<view
							...
							<view
								class="login__info__item__input__content login__info__item__input__content--verify-code">
								<input placeholder-class="input-placeholder" v-model="password" placeholder="请输入登录密码" />
							</view>
							<!-- 							<view class="login__info__item__input__right-verify-code" @tap.stop="getCode">
								<tn-button size="sm" padding="5rpx 10rpx" width="100%" shape="round">{{ tips }}
								</tn-button>
							</view> -->
						</view>
					...
					<view class="login__info__item__button tn-bg-blue tn-color-white" hover-class="tn-hover"
						:hover-stay-time="150" @click="login(currentModeIndex)">
				...
				<view class="login__way tn-flex tn-flex-col-center tn-flex-row-center">
					<view class="tn-padding-sm tn-margin-xs" @click="wxlogin">
						<view
			...
			login(n) {
				if (n == 0) {
					this.$http.user({
						...
						this.$http.show(err.msg)
					})
				} else {
					if (this.password != this.password_s) {
						this.$http.show('两次输入的密码不一致')
						return
					}
					this.$http.user({
						action: 'register',
